...
def get_data_file_info(video_path, suffix, filename=None):
    ...
    if os.path.exists(raw_yolo_path):
        ...
        if file_size <= 5: # prevent jsons with []
            log.warn(f"Raw YOLO data file is too small or empty: {raw_yolo_path} (size: {file_size} bytes)")
            try:
                os.remove(raw_yolo_path)
                log.info(f"Deleted small raw YOLO data file: {raw_yolo_path}")
            except OSError as e:
                log.error(f"Error deleting raw YOLO file {raw_yolo_path}: {e}")
