...
def tracking_analysis(state):
    ...
    if not exists:
        log.info(f"Raw yolo json not found in {raw_yolo_path}")
        return
    ...
    if first_penis_frame is None:
        log.error(f"No penis instance found in video. Further tracking is not possible.")
        return
    ...
    log.info(f"Frame Start adjusted to: {state.frame_start_track}")
    ...
    log.info(f"Finished processing video: {state.video_path}")
