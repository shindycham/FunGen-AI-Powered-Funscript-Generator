...
def render_debug_video_popup(window, state: AppState):
    ...
    debug_state = RenderDebugVideoState()
    ...
    Widgets.input(window, "Frame Start", state=debug_state, attr="frame_start", tooltip_text="Where to start processing the video. If you have a 60fps video starting at 10s would mean frame 600", row=2)
    Widgets.input(window, "Frame End", state=debug_state, attr="frame_end", tooltip_text="Where to end processing the video. If you have a 60fps video stopping  at 10s would mean frame 600", row=3)
    Widgets.button(
        window,
        "Generate",
